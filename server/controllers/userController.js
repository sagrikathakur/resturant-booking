...
export const registerUser = async (req, res) => {
  try {
    ...
    const user = await UserModel.createUser({ name, email, password: hashedPassword, phone });

    const token = jwt.sign({ id: user.id, email: user.email }, JWT_SECRET, { expiresIn: "7d" });
    const userData = { _id: user.id.toString(), id: user.id, name: user.name, email: user.email, phone: user.phone, role: "user" };
